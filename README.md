...
> 📦 **No antenna? No problem.** This repo includes a small sample database (`adsb_data.db`) with fabricated demo flight records, so you can run the backend and explore the map/UI right away — no RTL-SDR hardware or `dump1090` required. See [Running without hardware](#-running-without-hardware) below.
...
Don't have an RTL-SDR dongle or antenna handy? The repo ships with a sample `adsb_data.db`, so you can skip steps 2 and 3 above and go straight to:
...
**About the sample data:** these are not real ADS-B captures. `adsb_data.db` was generated by a small script that fabricates plausible-looking flights — made-up ICAO codes/callsigns (e.g. `AAL1234`) walked through a climb → cruise → descend altitude profile with a straight-line heading, at a location near Charlotte, NC. It exists purely so the historical-trips UI has something to display; it's not evidence of any real flight ever tracked by this app. The sample currently has 7 aircraft and 18 trips (143 rows total) with a mix of trip counts per plane (1 to 5) to show off the trip-browsing feature. Running `adsb_capture.py` against a real `dump1090` feed will overwrite it with genuine recordings.
...
- A small sample `adsb_data.db` with fabricated (not real) flight records is included in the repo so you can run and explore the app without an antenna — see [Running without hardware](#-running-without-hardware). If you capture your own data with `adsb_capture.py`, it will overwrite this file with real recordings.
